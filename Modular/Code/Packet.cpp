...
bool Packet::operator==(const Packet &other)
{
	...
	// Compare all relevant members
	bool val = (inputs == other.inputs &&
				neurons == other.neurons &&
				outputs == other.outputs &&
				commOutputs == other.commOutputs && // Compare commOutputs size
				randRange == other.randRange &&
				randChance == other.randChance &&
				midRepeats == other.midRepeats &&
				outSyns == other.outSyns &&
				commOutSyns == other.commOutSyns && // Compare commOutSyns
				Chance == other.Chance);
	if (val == false)
		return false;
	...
	for (int i = 0; i < neurons; i++)
	{
		...
		{
			int strength = Neurons[i].getSynapses()[j].getStrength();
			int otherStrength = (*((Packet)other).GetNeurons())[i].getSynapses()[j].getStrength();
			int toIndex = getNeuronIndex(Neurons[i].getSynapses()[j].getTo());
			int otherToIndex = ((Packet)other).getNeuronIndex((*((Packet)other).GetNeurons())[i].getSynapses()[j].getTo());
			val &= (strength == otherStrength && toIndex == otherToIndex);
			if (val == false)
				return false;
		}
		int bias = Neurons[i].getBias();
		int otherBias = (*((Packet)other).GetNeurons())[i].getBias();
		bool oprtr = Neurons[i].getOperator();
		bool otherOprtr = (*((Packet)other).GetNeurons())[i].getOperator();
		val &= (bias == otherBias && oprtr == otherOprtr);
		if (val == false)
			return false;
	}
...
Packet::Packet(Packet *packet)
{
	...
	this->firingNeurons.resize(inputs + neurons);
	for (int i = 0; i < inputs; i++)
	{
...
Packet *Packet::Clone()
{
	...
	clone->Inputs.resize(inputs);
	clone->Neurons.resize(neurons);
	clone->Outputs.resize(outputs);
	clone->CommOutputs.resize(commOutputs); // Allocate CommOutputs for clone
	clone->firingNeurons.resize(inputs + neurons);
	; // Resize firingNeurons to match the new size
	...
	for (int i = 0; i < inputs; i++)
	{
		for (int j = 0; j < Inputs[i].getNumSyns(); j++)
		{
			Neuron *targetOriginal = Inputs[i].getSynapses()[j].getTo();
			int synStrength = Inputs[i].getSynapses()[j].getStrength();
			int targetIdxOriginal = findNeuron(targetOriginal);
			int targetLayer = targetIdxOriginal > inputs ? (targetIdxOriginal > (inputs + neurons) ? (targetIdxOriginal > (inputs + neurons + outputs) ? 3 : 2) : 1) : 0;
			targetIdxOriginal -= targetLayer == 0 ? 0 : (targetLayer == 1 ? inputs : (targetLayer == 2 ? (inputs + neurons) : (inputs + neurons + outputs)));
			Neuron *targetClone = clone->getNeuron(targetLayer, targetIdxOriginal);
			clone->Inputs[i].MakeSynapse(synStrength, targetClone);
		}
	}
	for (int i = 0; i < neurons; i++)
	{
		for (int j = 0; j < Neurons[i].getNumSyns(); j++)
		{
			Neuron *targetOriginal = Neurons[i].getSynapses()[j].getTo();
			int synStrength = Neurons[i].getSynapses()[j].getStrength();
			int targetIdxOriginal = findNeuron(targetOriginal);
			int targetLayer = targetIdxOriginal > inputs ? (targetIdxOriginal > (inputs + neurons) ? (targetIdxOriginal > (inputs + neurons + outputs) ? 3 : 2) : 1) : 0;
			targetIdxOriginal -= targetLayer == 0 ? 0 : (targetLayer == 1 ? inputs : (targetLayer == 2 ? (inputs + neurons) : (inputs + neurons + outputs)));
			Neuron *targetClone = clone->getNeuron(targetLayer, targetIdxOriginal);
			clone->Neurons[i].MakeSynapse(synStrength, targetClone);
		}
	}

	// outputs have synapses within the greater network, so impossible to copy here unless passed the output layer
	// so it will simply be handled in the network class

	// for (int i = 0; i < outputs; i++) {
	// 	for (int j = 0; j < Outputs[i].getNumSyns(); j++) {
	// 		Neuron* targetOriginal = Outputs[i].getSynapses()[j].getTo();
	// 		int synStrength = Outputs[i].getSynapses()[j].getStrength();
	// 		int targetIdxOriginal = findNeuron(targetOriginal);
	// 		int targetLayer = targetIdxOriginal > inputs ? (targetIdxOriginal > (inputs + neurons) ? (targetIdxOriginal > (inputs + neurons + outputs) ? 3 : 2) : 1) : 0;
	// 		targetIdxOriginal -= targetLayer == 0 ? 0 : (targetLayer == 1 ? inputs : (targetLayer == 2 ? (inputs + neurons) : (inputs + neurons + outputs)));
	// 		Neuron* targetClone = clone->getNeuron(targetLayer, targetIdxOriginal);
	// 		clone->Outputs[i].MakeSynapse(synStrength, targetClone);
	// 	}
	// }

	return clone;
}
void Packet::CloneTo(Packet *clone)
{
	clone->packetId = this->packetId;
	clone->inputs = inputs;
	clone->neurons = neurons;
	clone->outputs = outputs;
	clone->commOutputs = commOutputs;
	clone->randRange = randRange;
	clone->randChance = randChance;
	clone->midRepeats = midRepeats;
	clone->outSyns = outSyns;
	clone->commOutSyns = commOutSyns;
	clone->Chance = Chance;

	clone->Inputs.resize(inputs);
	clone->Neurons.resize(neurons);
	clone->Outputs.resize(outputs);
	clone->CommOutputs.resize(commOutputs);
...
void Packet::Init(const std::string &id, int ins, int mid, int outs, int commOuts, int maxSyns, int outSynsParam, int commOutSynsParam, int repeats, int RandRange, int RandChance, int ChanceParam)
{
	...
	if (this->outSyns == 0 && neurons > 0 && outputs > 0)
		this->outSyns = 1;
	...
	if (this->commOutSyns == 0 && neurons > 0 && commOutputs > 0)
		this->commOutSyns = 1;
	...
	firingNeurons.resize(inputs + neurons);
...
void Packet::printSynapses(char *filter)
{
	...
	for (int i = 0; i < inputs; i++)
	{
		...
		for (int j = 0; j < Inputs[i].getNumSyns(); j++)
		{
			...
			{ // Add other layers if Inputs can connect to them
				layer_name = "Other";
				relative_idx = n_idx;
			}
...
int Packet::CreateSynapses(int maxSyns)
{
	...
	if (inputs > 0 && neurons > 0)
	{
		for (int i = 0; i < inputs; i++)
		{
			...
			if (synapseAmount == 0 && neurons > 0)
				synapseAmount = 1;
			if (synapseAmount > neurons)
				synapseAmount = neurons;
	...
	if (neurons > 0)
	{
		for (int i = 0; i < neurons; i++)
		{
			...
			if (synapseAmountTarget == 0 && neurons > 1)
				synapseAmountTarget = 1; // Ensure some connectivity if possible
			if (synapseAmountTarget > neurons)
				synapseAmountTarget = neurons;
			...
			for (int j = 0; j < synapseAmountTarget; j++)
			{
				...
				for (Neuron *existing_n : ns_vec)
					if (existing_n == n)
						exists = true;
				if (n == &Neurons[i])
					exists = true; // Don't connect to self in this simple setup
	...
	if (neurons > 0 && outputs > 0 && outSyns > 0)
	{
		...
		for (int i = 0; i < outSyns; ++i)
		{
			...
			bool exists = false;
			for (Neuron *existing_n : source_neurons_for_output)
				if (existing_n == n)
					exists = true;
			...
			if (!exists)
				source_neurons_for_output.push_back(n);
		}

		for (Neuron *source_neuron : source_neurons_for_output)
		{
			int numSynsToOut = outputs / 2 + (rand() % (outputs - outputs / 2 + 1));
			if (numSynsToOut == 0 && outputs > 0)
				numSynsToOut = 1;
			if (numSynsToOut > outputs)
				numSynsToOut = outputs;
			...
			for (int j = 0; j < numSynsToOut; j++)
			{
				...
				bool exists = false;
				for (Neuron *existing_n : target_output_neurons)
				...
				if (!exists)
					target_output_neurons.push_back(target_n);
			}
	...
	if (neurons > 0 && commOutputs > 0 && commOutSyns > 0)
	{
		...
		for (int i = 0; i < commOutSyns; ++i)
		{												// Select commOutSyns source neurons from Middle layer
			Neuron *n = getNeuron(1, rand() % neurons); // Layer 1 is Neurons
			bool exists = false;
			for (Neuron *existing_n : source_neurons_for_comm)
				if (existing_n == n)
					exists = true;
			...
			if (!exists)
				source_neurons_for_comm.push_back(n);
		}

		for (Neuron *source_neuron : source_neurons_for_comm)
		{
			int numSynsToComm = commOutputs / 2 + (rand() % (commOutputs - commOutputs / 2 + 1));
			if (numSynsToComm == 0 && commOutputs > 0)
				numSynsToComm = 1;
			if (numSynsToComm > commOutputs)
				numSynsToComm = commOutputs;
			...
			for (int j = 0; j < numSynsToComm; j++)
			{
				...
				bool exists = false;
				for (Neuron *existing_n : target_comm_neurons)
					if (existing_n == target_n)
						exists = true;
				...
				if (!exists)
					target_comm_neurons.push_back(target_n);
			}
...
void Packet::RunOnceCPU(int *a)
{
	...
	for (int i = 0; i < totalUNs; i++)
	{
		Neuron *n = nullptr;
		if (i < neurons)
		{ // Target is a middle Neuron
			n = getNeuron(1, i);
		}
		else if (i < neurons + outputs)
		{ // Target is an Output Neuron
			n = getNeuron(2, i - neurons);
		}
		else
		{ // Target is a CommOutput Neuron
...
void Packet::RunOnceGPU(int *a)
{
	...
	for (int i = 0; i < totalTargetableUNs; ++i)
	{
		...
		else
		{
			...
			if (synStrengthsHost[i].empty())
			{ // Should not happen if getSynStrengths guarantees [count, ...]
	...
	if (err != cudaSuccess)
	{
		std::cerr << "CUDA Malloc d_totals failed: " << cudaGetErrorString(err) << std::endl;
		return;
	}
	...
	if (err != cudaSuccess)
	{
		std::cerr << "CUDA Malloc d_flat_strengths_all failed: " << cudaGetErrorString(err) << std::endl;
		cudaFree(d_totals);
		return;
	}
	...
	if (err != cudaSuccess)
	{
		std::cerr << "CUDA Malloc d_from_pointers_array failed: " << cudaGetErrorString(err) << std::endl;
		cudaFree(d_totals);
	...
	if (err != cudaSuccess)
	{
		std::cerr << "CUDA Memcpy h_flat_strengths to d_flat_strengths_all failed: " << cudaGetErrorString(err) << std::endl; /* free memory */
		return;
	}
	...
	if (err != cudaSuccess)
	{
		std::cerr << "CUDA Memcpy h_device_pointers_for_d_from_array to d_from_pointers_array failed: " << cudaGetErrorString(err) << std::endl; /* free memory */
		return;
	}
	...
	if (err != cudaSuccess)
	{
		std::cerr << "CUDA Memcpy d_totals to totals failed: " << cudaGetErrorString(err) << std::endl; /* free memory */
		return;
	}
